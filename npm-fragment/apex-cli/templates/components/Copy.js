 ...
 */
export default function Copy({ children, animateOnScroll = false, delay = 0, className = "", stagger = 0.1, forceReady, }) {
    ...
    useEffect(() => {
        ...
        return () => {
            ...
            window.removeEventListener("languageTransitionComplete", handleLanguageReady);
            clearTimeout(timer);
        };
    ...
    useGSAP(() => {
        ...
        if (animateOnScroll) {
            ...
            const isInView = rect.top < window.innerHeight * 0.8 && rect.top > -rect.height;
            if (isInView) {
    ...
    return (_jsx("div", { ref: containerRef, "data-copy-wrapper": React.Children.count(children) > 1 ? "true" : "false", className: className, style: { opacity: isReady ? 1 : 0 }, children: children }));
}

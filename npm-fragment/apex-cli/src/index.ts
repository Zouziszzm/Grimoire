...
program
  ...
  .action(async (componentName) => {
    ...
    if (!registry[componentName]) {
      console.error(
        chalk.red(`Error: Component '${componentName}' not found in registry.`)
      );
      return;
    }
    ...
    const processedLocalDeps = new Set<string>();

    while (localDepsToProcess.length > 0) {
      const current = localDepsToProcess.shift()!;
      if (processedLocalDeps.has(current)) continue;
      ...
      comp.dependencies?.forEach((d: string) => componentsToInstall.add(d));

      // Add local dependencies to queue
      comp.localDependencies?.forEach((d: string) =>
        localDepsToProcess.push(d)
      );
      ...
      for (const file of comp.files) {
        ...
        if (await fs.pathExists(sourceFile)) {
          ...
          console.warn(
            chalk.yellow(`  ⚠ Source file ${sourceFile} not found.`)
          );
        }
      }
    }
    ...
    if (componentsToInstall.size > 0) {
      ...
      try {
        ...
        await execa(
          pkgManager,
          ["install", ...Array.from(componentsToInstall)],
          { stdio: "inherit" }
        );
